<template>
  <div class="couriers-container">
    ...
    <VCard class="couriers-header-card" elevation="2">
        ...
        <div class="d-flex flex-column flex-md-row align-center justify-space-between gap-4">
          <div class="header-title-section">
            <VCardTitle class="text-h4 font-weight-bold text-primary d-flex align-center gap-3">
              <VIcon icon="tabler-bike" size="32" color="primary" />
              Couriers Management
            </VCardTitle>
            <p class="text-body-2 text-medium-emphasis mt-2 mb-0">
              Manage your courier and their information
            </p>
          </div>
          ...
          <div class="d-flex flex-column flex-sm-row align-center gap-3">
            <VTextField
              ...
              clearable
            />
            <VBtn @click="addItem()" color="success" size="small" prepend-icon="tabler-plus" class="add-courier-btn">
              Add Courier
            </VBtn>
          </div>
        </div>
    ...
    <!-- Main Content -->
    <VCard class="couriers-content-card mt-6" elevation="1">
      <div v-if="couriers.length" class="couriers-table-container">
        <v-data-table
          :headers="headers"
          :items="couriers"
          item-key="id"
        ...
        >
          ...
          <template v-slot:[`item.photo`]="{ item }">
            <VAvatar size="48" class="courier-avatar">
          ...
          <template v-slot:[`item.vehicle_no`]="{ item }">
            <div class="d-flex align-center gap-1">
              <VIcon icon="tabler-truck" size="12" color="warning" />
              <span class="text-truncate" style="max-inline-size: 200px">{{ item.vehicle_no }}</span>
            </div>
          </template>
            ...
            <div class="modern-pagination pa-4">
              <div class="d-flex align-center justify-space-between flex-wrap gap-3">
                <VChip color="surface" variant="tonal" class="pagination-info">
                  {{ paginationMeta({ page, itemsPerPage }, couriers.length) }}
                </VChip>

                <VPagination 
                  v-model="page" 
                  :total-visible="5"
                  :length="Math.ceil(couriers.length / itemsPerPage)"
                  color="primary"
                  rounded
                >
        ...
        <VCard variant="tonal" class="text-center pa-8">
          <div class="loading-content">
            <VProgressCircular 
              ...
              class="mb-4"
            />
            <h3 class="text-h6 font-weight-medium mb-2">Loading couriers</h3>
            <p class="text-body-2 text-medium-emphasis">
              Please wait while we fetch the courier information...
            </p>
          </div>
        </VCard>
      </div>
      ...
      <VCard class="modern-dialog">
        <VCardTitle class="dialog-header pa-6">
          <div class="d-flex align-center gap-3">
            ...
            <span class="text-h5 font-weight-bold">Add New Courier</span>
          </div>
        </VCardTitle>
      ...
      <VCard class="modern-dialog">
        <VCardTitle class="dialog-header pa-6">
          <div class="d-flex align-center gap-3">
            ...
            <span class="text-h5 font-weight-bold">Edit Courier</span>
          </div>
        </VCardTitle>
              ...
              <VCol cols="12">
                <div class="photo-edit-section">
                  <div class="d-flex align-center gap-4 mb-3">
                    <VAvatar size="60">
                      <VImg :src="editedItem.photo" :alt="editedItem.name" cover />
                    </VAvatar>
                    <div>
                      <p class="text-subtitle-2 font-weight-medium mb-1">Current Photo</p>
                      <p class="text-caption text-medium-emphasis">Upload a new photo to replace</p>
                    </div>
                  </div>
                  <VFileInput 
                    v-on:change="onPhotoChange" 
    ...
    <VDialog v-model="deleteDialog" max-width="500px" persistent>
      <VCard class="modern-dialog delete-dialog">
        ...
        <VCardText class="text-center pa-6">
          <p class="text-body-1 mb-2">Are you sure you want to delete this courier?</p>
...
const couriersStore = useCourierStore();
onMounted(() => {
  couriersStore.fetchCouriers();
});

const couriers = computed(() => couriersStore.couriers);
...
const headers = [
  ...
  { title: 'Vehicle No', key: 'vehicle_no', sortable: false },
...
const rules = [
  fileList => !fileList || !fileList.length || fileList[0].size < 1000000 || 'Photo size should be less than 1 MB!'
]
...
const editItem = item => {
  editedIndex.value = couriers.value.indexOf(item)
...
const submitAdd = async () => { 
  ...
  formData.append("vehicle_no", editedItem.value.vehicle_no);
...
.couriers-header-card {
  border-radius: 16px !important;
}

.couriers-content-card {
  overflow: hidden;
  border-radius: 16px !important;
}
